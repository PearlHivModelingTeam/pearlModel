...
def run(group_name_run, replication_run):
    ...
    output_path1 = output_root_path/'csv_output'/f"{group_name_run}/bmi_{config['bmi_intervention']}/replication_{replication_run_str}"
    rerun_path = rerun_root_path/'csv_output'/f"group_name_run/replication_{replication_run_str}" if rerun_root_path is not None else None
