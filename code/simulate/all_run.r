...
######################################################################################
#' Read in models from R for time-varying processes
...
wrapper <- function(reps, groupname, sexvalue, prob_reengage, linelist) {
  ...
  art2009 <- art2009 %>%
    mutate(calyy = 2009) %>%
    select(group, sex, age, h1date, h1yy, sqrtcd4n, calyy)
  ...
  art2009 <- art2009 %>%
    left_join(coeff_cd4_increase %>% select(group, sex, coeffs) %>% unnest, by = c("group", "sex")) %>%
    ...
    mutate(pred = intercept_c + 
             ...
             (`_timecd4cat349_c`*time_from_h1yy_*cd4cat349) +
             (`_timecd4cat499_c`*time_from_h1yy_*cd4cat499) +
             (`_timecd4cat500_c`*time_from_h1yy_*cd4cat500) +
             (`__timecd4cat349_c`*time_from_h1yy__*cd4cat349) +
             (`__timecd4cat499_c`*time_from_h1yy__*cd4cat499) +
             (`__timecd4cat500_c`*time_from_h1yy__*cd4cat500) +
             (`___timecd4cat349_c`*time_from_h1yy___*cd4cat349) +
             (`___timecd4cat499_c`*time_from_h1yy___*cd4cat499) +
             (`___timecd4cat500_c`*time_from_h1yy___*cd4cat500)) %>%
    mutate(status = 0) 
  
  # Pull the variance-covariance matrix
  var_covar <- coeff_cd4_increase %>%
    semi_join(art2009, by = c("group", "sex")) %>%
    select(vcov)
  
  var_covar <- matrix(unlist(var_covar), ncol = 18, byrow = TRUE)
  
  # Pull the model & get the MSE
  modelref <- coeff_cd4_increase %>%
    semi_join(art2009, by = c("group", "sex")) %>%
    select(model)
  
  modelref <- unlist(modelref, recursive = F)
  
  dof <- df.residual(modelref$model)
  sig2 <- c(crossprod(modelref$model$residuals)) / dof # this is MSE - an estimate of error variance
  
  # Get se.fit
  CHK <- art2009 %>%
    mutate(intercept = 1,
           `_timecd4cat349` = time_from_h1yy_*cd4cat349, 
           `_timecd4cat499` = time_from_h1yy_*cd4cat499, 
           `_timecd4cat500` = time_from_h1yy_*cd4cat500, 
           `__timecd4cat349` = time_from_h1yy__*cd4cat349, 
           `__timecd4cat499` = time_from_h1yy__*cd4cat499, 
           `__timecd4cat500` = time_from_h1yy__*cd4cat500, 
           `___timecd4cat349` = time_from_h1yy___*cd4cat349, 
           `___timecd4cat499` = time_from_h1yy___*cd4cat499, 
           `___timecd4cat500` = time_from_h1yy___*cd4cat500) %>%
    select(patid, intercept, time_from_h1yy, time_from_h1yy_,  time_from_h1yy__,  time_from_h1yy___,
           cd4cat349, cd4cat499, cd4cat500, agecat, `_timecd4cat349`, `_timecd4cat499`, `_timecd4cat500`, 
           `__timecd4cat349`, `__timecd4cat499`, `__timecd4cat500`, `___timecd4cat349`, `___timecd4cat499`, `___timecd4cat500`)
  
  # Convert to a matrix & drop patid
  #CHK2 <- data.matrix(CHK[,-1])
  CHK2 <- CHK[,-1]
  
  # Convert to a list of vectors
  CHK2 <- as.list(data.frame(t(CHK2)))
  
  # Get the SEs
  se_function <- function(x) {
    std.er <- sqrt(t(x) %*% var_covar %*% x)
    std.er.pi <- sqrt(std.er^2 + sig2)
  }
  
  CHK2 <- lapply(CHK2, FUN = se_function)
  
  # Convert back to a data frame
  CHK2 <- data.frame(se = matrix(unlist(CHK2), nrow=length(CHK2), byrow=T))
  CHK <- bind_cols(CHK %>% select(patid), CHK2)
  
  art2009 <- art2009 %>%
    left_join(CHK, by = "patid")
  
  # Generate prediction from prediction interval & set min and max values
  art2009 <- art2009 %>%
    mutate(tv_sqrtcd4n = runif(nrow(art2009), min = pred - 1.96*se, max = pred + 1.96*se),
           tv_sqrtcd4n = replace(tv_sqrtcd4n, tv_sqrtcd4n < 0, 0),
           tv_sqrtcd4n = replace(tv_sqrtcd4n, tv_sqrtcd4n > 50, 50))
  ...
  #############################
  newart <- test %>%
    select(group, sex, new_art_mixture) %>%
    unnest %>%
    select(group, sex, age = iniage, h1date, h1yy = H1YY, sqrtcd4n) %>%
    mutate(calyy = h1yy,
  ...
  pop1 <- pop1 %>%
    ...
    left_join(coeff_leave_na %>% select(group, sex, coeffs) %>% unnest, by = c("group", "sex")) %>%
  ...
  # Pull the variance-covariance matrix
  var_covar <- coeff_leave_na %>%
    semi_join(pop1, by = c("group", "sex")) %>%
    select(vcov)
  
  var_covar <- matrix(unlist(var_covar), ncol = 8, byrow = TRUE)
  
  # Pull the model
  modelref <- coeff_leave_na %>%
    semi_join(art2009, by = c("group", "sex")) %>%
    select(model)
  ...
  dof <- df.residual(modelref$model)
  sig2 <- c(crossprod(modelref$model$residuals)) / dof
  
  # Get se.fit
  CHK <- pop1 %>%
    mutate(intercept = 1) %>%
    select(patid, intercept, age, `age_`, `age__`, `age___`, calyy, sqrtcd4n, haart_period) 
  ...
  pop1 <- pop1 %>%
    mutate(y2 = runif(nrow(pop1), min = y - 1.96*se, max = y + 1.96*se),
           prob = exp(y2) / (1 + exp(y2)),
           prob = replace(prob, prob < 0, 0),
           prob = replace(prob, prob > 1, 1),
           rr = runif(nrow(pop1)),
           status = 0,
  ...
  for (YY in seq_along(seqyears)) {
    ...
    pop1 <- pop1 %>%
      mutate(tv_sqrtcd4n = runif(nrow(pop1), min = pred - 1.96*se, max = pred + 1.96*se),
             tv_sqrtcd4n = replace(tv_sqrtcd4n, tv_sqrtcd4n < 0, 0),
             tv_sqrtcd4n = replace(tv_sqrtcd4n, tv_sqrtcd4n > 50, 50))
    ...
    pop1 <- pop1 %>%
      left_join(coeff_mortality_in %>% select(group, sex, coeffs) %>% unnest, by = c("group", "sex")) %>%
      mutate(y = intercept_c + (agecat_c*agecat) + (sqrtcd4n_c*sqrtcd4n) + (year_c*calyy) + (h1yy_c*h1yy))
    
    # Pull the variance-covariance matrix
    var_covar <- coeff_mortality_in %>%
      semi_join(pop1, by = c("group", "sex")) %>%
      select(vcov)
    
    var_covar <- matrix(unlist(var_covar), ncol = 5, byrow = TRUE)
    
    # Pull the model
    modelref <- coeff_mortality_in %>%
      semi_join(art2009, by = c("group", "sex")) %>%
      select(model)
    ...
    CHK <- pop1 %>%
      mutate(intercept = 1) %>%
      select(patid, intercept, calyy, agecat, sqrtcd4n, h1yy)
    ...
    pop1 <- pop1 %>%
      mutate(y2 = runif(nrow(pop1), min = y - 1.96*se, max = y + 1.96*se),
             ...
             rr = runif(nrow(pop1)),
             status = 0,
             status = replace(status, rr <= prob, 1),
             status = replace(status, age > 85, 1)) %>%
      select(-y, -y2, -prob, -rr, -se, -intercept_c,  -year_c, -agecat_c, -sqrtcd4n_c, -h1yy_c)
    ...
    # POP2: OUT OF CARE
    #################################
    keep_names_final <- c("group", "sex", "age", "h1date", "h1yy", "sqrtcd4n", "calyy", "patid", "agecat", "tv_sqrtcd4n", "status", "ltfu_year", "sqrtcd4n_exit")
    ...
    coeff <- data.frame(coeff_cd4_decrease$coeff)
    
    pop2 <- cbind(pop2, coeff)
    
    pop2 <- pop2 %>%
      mutate(time_out = calyy - ltfu_year) %>%
      mutate(pred = intercept_c + time_out_of_naaccord_c*time_out + sqrtcd4_exit_c*sqrtcd4n_exit)
    
    # Pull the variance-covariance matrix
    var_covar <- data.frame(coeff_cd4_decrease$vcov)
    
    var_covar <- matrix(unlist(var_covar), ncol = 3, byrow = TRUE)
    
    # Pull the model
    modelref <- coeff_cd4_decrease$model
    
    dof <- df.residual(modelref)
    sig2 <- c(crossprod(modelref$residuals)) / dof
    
    # Get se.fit
    CHK <- pop2 %>%
      mutate(intercept = 1) %>%
      select(patid, intercept, time_out, sqrtcd4n_exit)
    ...
    pop2 <- pop2 %>%
      mutate(pred2 = runif(nrow(pop2), min = pred - 1.96*se, max = pred + 1.96*se),
             tv_sqrtcd4n = sqrt((sqrtcd4n_exit**2)*exp(pred2)*1.5),
             tv_sqrtcd4n = replace(tv_sqrtcd4n, tv_sqrtcd4n < 0, 0),
             tv_sqrtcd4n = replace(tv_sqrtcd4n, tv_sqrtcd4n > 50, 50)) %>%
      select(keep_vars)
    ...
    pop2 <- pop2 %>%
      left_join(coeff_mortality_out %>% select(group, sex, coeffs) %>% unnest, by = c("group", "sex")) %>%
      mutate(y = intercept_c + (year_c*calyy) + (agecat_c*agecat) + (tv_sqrtcd4n_c*tv_sqrtcd4n))
    
    # Pull the variance-covariance matrix
    var_covar <- coeff_mortality_out %>%
      semi_join(pop2, by = c("group", "sex")) %>%
      select(vcov)
    
    var_covar <- matrix(unlist(var_covar), ncol = 4, byrow = TRUE)
    
    # Pull the model
    modelref <- coeff_mortality_out %>%
      semi_join(art2009, by = c("group", "sex")) %>%
      select(model)
    ...
    CHK <- pop2 %>%
      mutate(intercept = 1) %>%
      select(patid, intercept, calyy, agecat, tv_sqrtcd4n)
    ...
    pop2 <- pop2 %>%
      mutate(y2 = runif(nrow(pop2), min = y - 1.96*se, max = y + 1.96*se),
             prob = exp(y2) / (1 + exp(y2)),
             prob = replace(prob, prob < 0, 0),
             prob = replace(prob, prob > 1, 1),
             rr = runif(nrow(pop2)),
