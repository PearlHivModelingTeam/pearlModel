#' 05/17/19: non-varying parameters
...
fx1 <- function(DF, naaccordwd) {
  ...
  # Updated 04/24/19
  carestat <- read_sas(paste0(naaccordwd,"/popu16_carestatus.sas7bdat"))
...
######################################################################################
# Decide between weibull and mixture and simulate final 2009 population
fx6 <- function(mixture_2009, naaccord_2009, on_art, naaccord_prop_2009, naaccord_cd4_2009) {
  ...
  components <- sample(1:2, prob=c(mixture_2009$lambda1, mixture_2009$lambda2), size=on_art, replace=TRUE)
  
  n_pop1 <- length(components[components==1])
  pop1 <- data.frame(age2009 = rtnorm_age(n_pop1, mixture_2009$mu1, mixture_2009$sigma1))
  
  n_pop2 <- length(components[components==2])
  pop2 <- data.frame(age2009 = rtnorm_age(n_pop2, mixture_2009$mu2, mixture_2009$sigma2))
