...
with pd.HDFStore(param_dir + '/parameters.h5') as store:
    data = store['cd4n_by_h1yy']

data_raw = feather.read_dataframe(f'{param_dir}/aim1/cd4n_by_h1yy_raw.feather')

year_1 = [x for x in range(2010, 2021)]
year_2 = [x for x in range(2020, 2031)]
year_2020 = len(year_2) * [2020]
year_1 = np.array(year_1)
year_2 = np.array(year_2)
    ...
    for row, (cols, param) in enumerate(zip(axes, params)):
        for col, (ax, group) in enumerate(zip(cols, risk_list)):
            ...
            if row==0:
                ...
                ax.scatter(df_raw['period'], df_raw['mean'], color='k', label='NA-ACCORD')
                ax.plot(year_1, df['meanint'] + df['meanslp'] * year_1, color = colors[row])
                ax.plot(year_2, df['meanint'] + df['meanslp'] * year_2020, color = colors[row], label='Predicted $\mu$')
                #ax.set_ylim(15.0, 30.0)
            if row==1:
                ax.scatter(df_raw['period'], df_raw['sd'], color='k', label='NA-ACCORD')
                ax.plot(year_1, df['stdint'] + df['stdslp'] * year_1, color = colors[row])
                ax.plot(year_2, df['stdint'] + df['stdslp'] * year_2020, color = colors[row], label='Predicted $\sigma$')
