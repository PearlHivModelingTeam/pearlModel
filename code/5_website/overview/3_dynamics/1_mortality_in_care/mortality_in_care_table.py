...
data = data.rename(columns={'group': 'Population',
                            ...
                            'age_cat': '$\\beta_\mathrm{age\_cat}$',
                            'sqrtcd4n': '$\\beta_\mathrm{sqrt\_init\_cd4}$',
                            'h1yy': '$\\beta_\mathrm{art\_init\_year}$'})
data = data.round(decimals=3)
data.to_excel(f'out/mortality_in_care_table.xlsx', index=False)
colWidths = [2.6] + 4 *[1.5]
colWidths = 6 * [5]

render_mpl_table(data, header_columns=0, col_width=2.6, header_color='#347DBE', colWidths=colWidths)
plt.savefig(f'out/mortality_in_care_table.png', bbox_inches='tight', dpi=300)
