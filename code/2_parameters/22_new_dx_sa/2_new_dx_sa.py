...
new_dx_int = pd.read_csv(f'{intermediate_dir}/new_dx_all.csv').assign(rm=0)
new_dx_int_list = []
for i, year in enumerate(range(2009, 2015)):
    new_dx_int_list.append(pd.read_csv(f'{intermediate_dir}/new_dx_{year}.csv').assign(rm=i+1))
