...
df = pd.read_csv(f'{in_dir}/mortality_in_care_coeffs_R.csv')
df.columns = df.columns.str.lower()
covariates = ['intercept', 'year', 'age', 'age1', 'age2', 'sqrtcd4n', 'sqrtcd41', 'sqrtcd42', 'h1yy_cat_1', 'h1yy_cat_2']
...
df = pd.read_csv(f'{in_dir}/mortality_in_care_vcov_R.csv')
df = df.rename({'Intercept': 'intercept',
                'rowname': 'covariate',
                'h1yy_cat2009-2012': 'h1yy_cat_1',
                'h1yy_cat2013-2017': 'h1yy_cat_2'}, axis='columns')
df['covariate'] = df['covariate'].replace(to_replace={'Intercept': 'intercept',
                                                      'h1yy_cat2009-2012': 'h1yy_cat_1',
                                                      'h1yy_cat2013-2017': 'h1yy_cat_2'})
df1 = df.loc[df['group'] == 'idu_hisp + idu_white_women'].copy()
df1 = pd.concat([df1.assign(group='idu_hisp_women'), df1.assign(group='idu_white_women')])
...
df.to_csv(f'{out_dir}/mortality_in_care_vcov.csv', index=False)
