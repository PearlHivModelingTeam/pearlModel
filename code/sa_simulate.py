...
if args.config:
    ...
    output_folder = pearl_dir/f'out/{config_file.stem}_sa_{date_string}'
elif args.rerun:
...
if os.path.isdir(output_folder):
    ...
    for key in sa_dict:
        for i in [0, 1]:
            output_folder_sa = output_folder/f'{key}_{i}'
...
for key in sa_dict:
    for i in [0, 1]:
        ...
        if rerun_folder is not None:
            rerun_folder_sa = rerun_folder/f'{key}_{i}'
        else:
        ...
        for group_name in group_names:
            ...
            parameters = pearl.Parameters(path=param_file, rerun_folder=rerun_folder_sa, output_folder=output_folder_sa, group_name=group_name, comorbidity_flag=comorbidity_flag,
                                          mm_detail_flag=mm_detail_flag, new_dx=new_dx, final_year=final_year, mortality_model=mortality_model,
                                          mortality_threshold_flag=mortality_threshold_flag, verbose=verbose, sa_dict=sa_dict_run)
