...
def draw_from_trunc_norm(a, b, mu, sigma, n):
    """Return a numpy array filled with n values drawn from a truncated normal with the given parameters. If n=0 return an empty list."""
    y = []
    if n != 0:
        a_mod = (a - mu) / sigma
        b_mod = (b - mu) / sigma
...
class Pearl:
    ...
    def make_pop_2009(self, n_initial_nonusers):
        ...
        and 85. ART initiation year is assigned using proportions from NA-ACCORD data. Sqrt CD4 count is drawn from a normal
    ...
    def make_new_population(self, n_new_agents):
        ...
        self.stats.art_coeffs = self.parameters.age_by_h1yy[['estimate']].assign(group=self.group_name, replication=self.replication, variable='age').reset_index()
        ...
        art_coeffs_cd4 = self.parameters.cd4n_by_h1yy[['estimate']].assign(group=self.group_name, replication=self.replication, variable='cd4').reset_index()
        self.stats.art_coeffs = pd.concat([self.stats.art_coeffs, art_coeffs_cd4])
    ...
    def run(self):
        ...
        while self.year <= 2030:
            ...
            # Everybody calendar year, ages, age_cat and years out of care
    ...
    def record_stats(self):
        ...
        statistics for both in and out of care populations. If it is an Aim 2 simulation, record the prevalence of all comorbidities
        and the multimorbidity for the in care, out of care, initiating, and dying populations. If the multimorbidity detail flag is set,
        ...
        if self.parameters.comorbidity_flag:
            ...
            # If the flag is set, record the detailed comorbidity information
...
class Parameters:
    """This class holds all of the parameters needed for PEARL to run."""
