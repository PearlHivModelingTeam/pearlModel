...
if args.config:
    yaml_file = Path(f'sa_config/{args.config}')
else:
    yaml_file = Path('sa_config/sa_test.yaml')

date_string = datetime.today().strftime('%Y-%m-%d')
output_folder_base = Path(f'../../out/{yaml_file.stem}_{date_string}/')
...
with open(yaml_file, 'r') as f:
    ...
    sa_dict_base = param_yaml['sa_dict']
...
for key in sa_dict_base:
    for i in [0, 1]:
        output_folder = f'{output_folder_base}/{key}_{i}'
        sa_dict = sa_dict_base.copy()
        sa_dict[key] = i
