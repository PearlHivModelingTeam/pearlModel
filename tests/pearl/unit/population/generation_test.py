...
from pearl.population.generation import (simulate_ages,
                                         simulate_new_dx,
...
def test_simulate_ages_coeffs():
    coeffs = {
        ...
        'estimate' : [0.1428275968730224,
                      25.90157466563675,
                      44.15102821208629,
                      3.36447304669819,
                      8.995675326063536,],
        'conf_high' : [0.1696263422876576,
                       26.46764195909201,
                       44.7052307739297,
                       3.8295935710199736,
                       9.373568241855462],
        'conf_low' : [0.1160288514583873,
                      25.335507372181493,
                      43.59682565024288,
                      2.8993525223764065,
                      8.617782410271609]
...
@fixture
def expected_ages():
    return np.array([59.01796147725942,
                     49.731344366913646,
                     46.415830971908484,
                     35.114459279477444,
                     35.1135600789522])
...
def test_simulate_new_dx(test_new_dx, test_linkage_to_care, random_state, expected_n_initial_nonusers, expected_new_agents):
    '''
    It should return the same numbers when passed a seeded RandomState
    '''
    initial_nonusers, new_agents = simulate_new_dx(test_new_dx, test_linkage_to_care, random_state=random_state)
...
def test_apply_bmi_intervention_scenario_one(test_bmi_population, test_bmi_intervention_parameters, random_state, expected_bmi_intervention_output_scenario_one):
    '''
    ...
    bmi_output_scenario_one = apply_bmi_intervention(test_bmi_population, test_bmi_intervention_parameters, random_state=random_state)
    
    assert_series_equal(bmi_output_scenario_one['post_art_bmi'], expected_bmi_intervention_output_scenario_one)
...
def test_apply_bmi_intervention_scenario_two(test_bmi_population, test_bmi_intervention_parameters, random_state, expected_bmi_intervention_output_scenario_two):
    '''
    It should return the same population when passed a seeded random_state.
    '''
    test_bmi_intervention_parameters.bmi_intervention_scenario = 2
    bmi_output_scenario_two = apply_bmi_intervention(test_bmi_population, test_bmi_intervention_parameters, random_state=random_state)
    
    assert_series_equal(bmi_output_scenario_two['post_art_bmi'], expected_bmi_intervention_output_scenario_two)
...
def test_apply_bmi_intervention_scenario_three(test_bmi_population, test_bmi_intervention_parameters, random_state, expected_bmi_intervention_output_scenario_three):
    '''
    It should return the same population when passed a seeded random_state.
    '''
    test_bmi_intervention_parameters.bmi_intervention_scenario = 3
    bmi_output_scenario_three = apply_bmi_intervention(test_bmi_population, test_bmi_intervention_parameters, random_state=random_state)
    
    assert_series_equal(bmi_output_scenario_three['post_art_bmi'], expected_bmi_intervention_output_scenario_three)
...
def test_art_bmi_population():
    population = {
        ...
        'init_sqrtcd4n' : pd.Series([11.038756528100167,
                                     23.32942023071808,
                                     23.73959591750536,
                                     20.67559217085155,
                                     18.336389525364968]),
        'last_init_sqrtcd4n' : pd.Series([7.785768284900243,
                                          13.921099280684002,
                                          6.670803203549613,
                                          3.962003815707643,
                                          11.188837871609948]),
...
def expected_post_art_bmi_no_intervention():
    return np.array([29.802222136230082, 22.51843690121106, 26.695163432294805, 14.856891053958686, 28.78478441725878])

def test_calculate_post_art_bmi_no_intervention(test_art_bmi_population,
    ...
    '''
    post_art_bmi = calculate_post_art_bmi(test_art_bmi_population,
                                          test_art_bmi_parameters,
...
def expected_post_art_bmi_with_intervention():
    return np.array([29.130797492808735, 22.51837077835867, 26.56599902564644, 14.856891053955207, 27.39921848297732])

def test_calculate_post_art_bmi_with_intervention(test_art_bmi_population,
                                                  test_art_bmi_parameters,
                                                  random_state,
...
def expected_pre_art_bmi():
    return np.array([28.91516451518173, 27.361387946712178, 26.029469342376085, 20.654728481445687, 19.85041131111226])

def test_calculate_pre_art_bmi(test_art_bmi_population,
                               test_art_bmi_parameters,
                               random_state,
                               expected_post_art_bmi_with_intervention):
    '''
    It should return the same output, given the same input and a seeded RandomState with no intervention.
    '''
    pre_art_bmi = calculate_pre_art_bmi(test_art_bmi_population,
                                          test_art_bmi_parameters,
                                          random_state=random_state)
