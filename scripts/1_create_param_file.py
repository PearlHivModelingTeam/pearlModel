...
on_art_2009 = (
    pd.read_csv(param_dir / "initial_population_2009/population_size/on_art_2009.csv")
    .set_index(["group"])
    .sort_index()
)
...
h1yy_by_age_2009 = h1yy_by_age_2009.reindex(
    pd.MultiIndex.from_product(
        [
            h1yy_by_age_2009.index.levels[0].unique(),
...
for group in h1yy_by_age_2009.index.levels[0].unique():
    for age_cat in h1yy_by_age_2009.index.levels[1].unique():
        if h1yy_by_age_2009.loc[group, age_cat].isnull().values.any():
            h1yy_by_age_2009.loc[group, age_cat] = h1yy_by_age_2009.loc[
                "msm_white_male", age_cat
            ].values
...
df = pd.DataFrame(
    index=pd.MultiIndex.from_product(
        [group_names, ["mu", "sigma"], years], names=["group", "param", "h1yy"]
    ),
    columns=["estimate"],
).sort_index()
for group in group_names:
    mu = pd.DataFrame(
        {
            "h1yy": years,
            "estimate": cd4n_by_h1yy_2009.loc[group, "meanint"]
            + cd4n_by_h1yy_2009.loc[group, "meanslp"] * years,
        }
    )
    ...
    sigma = pd.DataFrame(
        {
            "h1yy": years,
            "estimate": cd4n_by_h1yy_2009.loc[group, "stdint"]
            + cd4n_by_h1yy_2009.loc[group, "stdslp"] * years,
        }
    )
...
new_dx = pd.read_csv(
    param_dir / "new_diagnosis_population/population_size/new_dx_interval_updated.csv"
).set_index(["group", "year"])
new_dx_ehe = pd.read_csv(
    param_dir / "new_diagnosis_population/population_size/new_dx_combined_ehe.csv"
).set_index(["group", "year"])
new_dx_sa = pd.read_csv(
    param_dir / "new_diagnosis_population/population_size/new_dx_interval_sa.csv"
).set_index(["group", "year"])

# Linkage to care
linkage_to_care = pd.read_csv(
    param_dir / "new_diagnosis_population/population_size/linkage_to_care.csv"
).set_index(["group", "year"])
...
age_by_h1yy["value_2018"] = np.array(
    [total_years * [value_2018] for value_2018 in values_2018]
).flatten()
# Create range after 2018
age_by_h1yy["le_2018"] = age_by_h1yy["h1yy"] <= 2018
age_by_h1yy["pred2"] = (
    age_by_h1yy["le_2018"].astype(int) * age_by_h1yy["pred"]
    + (~age_by_h1yy["le_2018"]).astype(int) * age_by_h1yy["value_2018"]
)
age_by_h1yy["low_value"] = age_by_h1yy[["pred", "pred2"]].min(axis=1)
age_by_h1yy["high_value"] = age_by_h1yy[["pred", "pred2"]].max(axis=1)
age_by_h1yy = (
    age_by_h1yy[["group", "param", "h1yy", "low_value", "high_value"]]
...
cd4n_by_h1yy = (
    pd.read_csv(param_dir / "new_diagnosis_population/cd4_at_art_init/cd4n_by_h1yy.csv")
    .set_index("group")
    .sort_index()
)
...
df = pd.DataFrame(
    index=pd.MultiIndex.from_product(
        [group_names, params, years], names=["group", "param", "h1yy"]
    ),
    columns=["low_value", "high_value"],
).sort_index()
for group in group_names:
    ...
    mu = pd.DataFrame(
        {
            "h1yy": years,
            "low_value": cd4n_by_h1yy.loc[group, "meanint"]
            + cd4n_by_h1yy.loc[group, "meanslp"] * years,
        }
    )
    ...
    sigma = pd.DataFrame(
        {
            "h1yy": years,
            "low_value": cd4n_by_h1yy.loc[group, "stdint"]
            + cd4n_by_h1yy.loc[group, "stdslp"] * years,
        }
    )
...
mortality_in_care_sqrtcd4 = pd.read_csv(
    param_dir
    / "population_dynamics/mortality_in_care/mortality_in_care_sqrtcd4_by_sex_race_risk.csv"
).set_index("group")
mortality_in_care_vcov = pd.read_csv(
    param_dir / "population_dynamics/mortality_in_care/mortality_in_care_vcov.csv"
).set_index(["group", "covariate"])

mortality_threshold = pd.read_csv(
    param_dir / "population_dynamics/mortality_threshold/cdc_mortality.csv"
...
mortality_out_care_age = pd.read_csv(
    param_dir
    / "population_dynamics/mortality_out_care/mortality_out_care_age_by_sex_race_risk.csv"
).set_index("group")
mortality_out_care_tv_sqrtcd4 = pd.read_csv(
    param_dir
    / "population_dynamics/mortality_out_care/mortality_out_care_tv_sqrtcd4_by_sex_race_risk.csv"
).set_index("group")
mortality_out_care_vcov = pd.read_csv(
    param_dir / "population_dynamics/mortality_out_care/mortality_out_care_vcov.csv"
).set_index(["group", "covariate"])

# Coefficients for loss to follow up
loss_to_follow_up = pd.read_csv(
    param_dir / "population_dynamics/disengagement/loss_to_follow_up.csv"
)
cols = loss_to_follow_up.columns.tolist()
loss_to_follow_up = loss_to_follow_up.set_index("group")
loss_to_follow_up_vcov = pd.read_csv(
    param_dir / "population_dynamics/disengagement/loss_to_follow_up_vcov.csv"
)
...
cd4_decrease = pd.read_csv(
    param_dir / "population_dynamics/cd4_dynamics_out_care/cd4_decrease.csv"
)
cols = cd4_decrease.columns.tolist()
cd4_decrease["group"] = "all"
cd4_decrease = cd4_decrease.set_index("group")
cd4_decrease_vcov = pd.read_csv(
    param_dir / "population_dynamics/cd4_dynamics_out_care/cd4_decrease_vcov.csv"
)
...
cd4_increase_vcov = pd.read_csv(
    param_dir / "population_dynamics/cd4_dynamics_in_care/cd4_increase_vcov.csv"
)
...
cd4_increase_knots = pd.DataFrame(
    {
        "group": group_names,
        "p5": 15 * [1.0],
        "p35": 15 * [4.0],
...
years_out_of_care = pd.read_csv(
    param_dir / "population_dynamics/reengagement/years_out_of_care.csv"
)
...
dpr_coeff = (
    pd.read_csv(param_dir / "aim2/stage1/depression_coeff.csv")
    .set_index(["group", "param"])
    .unstack()
)
...
mortality_in_care_co = (
    pd.read_csv(param_dir / "aim2/mortality/mortality_in_care.csv")
    .set_index(["group", "param"])
    .unstack()
)
mortality_in_care_post_art_bmi = pd.read_csv(
    param_dir / "aim2/mortality/mortality_in_care_post_art_bmi.csv"
).set_index("group")

mortality_out_care_co = (
    pd.read_csv(param_dir / "aim2/mortality/mortality_out_care.csv")
    .set_index(["group", "param"])
    .unstack()
)
mortality_out_care_post_art_bmi = pd.read_csv(
    param_dir / "aim2/mortality/mortality_out_care_post_art_bmi.csv"
).set_index("group")

# Mortality paper options
mortality_in_care_overall = pd.read_csv(
...
mortality_in_care_by_sex = pd.read_csv(
    param_dir / "population_dynamics/mortality_in_care/mortality_in_care_by_sex.csv"
).set_index("group")
mortality_in_care_age_by_sex = pd.read_csv(
...
mortality_out_care_tv_sqrtcd4_by_sex_race = pd.read_csv(
    param_dir
    / "population_dynamics/mortality_out_care/mortality_out_care_tv_sqrtcd4_by_sex_race.csv"
).set_index("group")
mortality_threshold_by_sex_race = pd.read_csv(
    param_dir / "population_dynamics/mortality_threshold/cdc_mortality_by_sex_race.csv"
).set_index(["group", "mortality_age_group"])

mortality_threshold_idu_1x = pd.read_csv(
    param_dir / "population_dynamics/mortality_threshold/cdc_mortality_idu_1x.csv"
).set_index(["group", "mortality_age_group"])
mortality_threshold_idu_5x = pd.read_csv(
    param_dir / "population_dynamics/mortality_threshold/cdc_mortality_idu_5x.csv"
).set_index(["group", "mortality_age_group"])
mortality_threshold_idu_10x = pd.read_csv(
    param_dir / "population_dynamics/mortality_threshold/cdc_mortality_idu_10x.csv"
).set_index(["group", "mortality_age_group"])
